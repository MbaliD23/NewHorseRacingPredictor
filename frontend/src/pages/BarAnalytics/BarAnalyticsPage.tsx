...
export function BarAnalyticsPage() {
  ...
  useEffect(() => {
    if (isRaceActive && dynamicHorses.length > 0) {
      setActiveHorseIds((prev) => {
        ...
        if (valid.length > 0) return valid.slice(0, BAR_MAX);
        return dynamicHorses.slice(0, BAR_MAX).map((h) => h.id);
      });
  ...
  /* Toggle horse selection: max 5, keep at least 1 */
  ...
  const barLimitReached = activeHorseIds.length >= BAR_MAX;
          ...
          dynamicHorses.map((horse, idx) => {
            ...
            return (
              <button
                ...
                title={isSelected ? `Remove ${horse.name} from bar chart` : `Add ${horse.name} to bar chart (max 5)`}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 5,
          ...
          <div className={styles.chartBody}>
            <div className={styles.chartCanvas}>
              {!isRaceActive ? (
                ...
                <BarChart activeHorseIds={activeHorseIds} activeAxes={activeAxes} horses={dynamicHorses} />
              )}
            </div>
          </div>
        </div>
        ...
        <div className={styles.bottomCard}>
          <div className={styles.tableHeader}>
            ...
            <span className={styles.tableHint}>
              ★ Click row to toggle horse on bar chart (max 5)
            </span>
          </div>
          ...
          {!isRaceActive ? (
            ...
            <div className={styles.tableScroll}>
              <table
                ...
                role="grid"
                aria-label="Race record comparison – click row to toggle horse on bar chart"
              >
                <thead>
                ...
                <tbody>
                  {dynamicHorses.map((horse, idx) => {
                    ...
                    const isLimitedOut = barLimitReached && !isActive;
...
export default BarAnalyticsPage;
